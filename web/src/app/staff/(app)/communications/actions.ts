...
export type SaveTemplateInput = {
  ...
  /** WhatsApp extras — header text, footer, buttons (image key managed separately). */
  rich?: CommTemplate["rich"];
};
...
export async function saveTemplateAction(input: SaveTemplateInput): Promise<CommActionState<CommTemplate>> {
  ...
  if (input.channel === "whatsapp" && input.kind === "text") payload.rich = input.rich ?? null;
...
/** Upload (or replace) a template's header image — JPEG/PNG, max 5 MB. */
export async function uploadTemplateHeaderImageAction(
  id: string,
  formData: FormData
): Promise<CommActionState<CommTemplate>> {
  const file = formData.get("file");
  if (!(file instanceof File) || file.size === 0) return { ok: false, error: "Choose an image file." };
  if (!/^image\/(jpeg|png)$/i.test(file.type)) return { ok: false, error: "Header images must be JPEG or PNG." };
  if (file.size > 5 * 1024 * 1024) return { ok: false, error: "Header images must be under 5 MB." };
  const data = Buffer.from(await file.arrayBuffer()).toString("base64");
  const result = await coreApi<CommTemplate>(`/templates/${id}/header-image`, {
    method: "POST",
...
export async function removeTemplateHeaderImageAction(id: string): Promise<CommActionState<CommTemplate>> {
  const result = await coreApi<CommTemplate>(`/templates/${id}/header-image`, {
    method: "POST",
    body: { remove: true }
  });
