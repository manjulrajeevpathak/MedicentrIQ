...
// The patient page fetches core-api from BOTH the server (initial render) and the
// browser (confirm / reschedule / upload). The browser can't reach the internal
// docker hostname, so client calls go through a same-origin proxy path that a Next
// rewrite forwards to core-api (see next.config.mjs) — keeping core-api private.
const coreApiUrl =
  typeof window === "undefined" ? process.env.NEXT_PUBLIC_CORE_API_URL : "/care/api/core";
