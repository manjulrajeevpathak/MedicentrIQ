...
import { addConditionAction, type ActionResult } from "@/app/clinician/actions";
import type { ClinicalRecord, ConditionOption } from "@clinician/lib/types";
...
function AddConditionForm({ patientId, onDone }: { patientId: string; onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    debounce.current = setTimeout(async () => {
      try {
        const res = await fetch(`/clinician/api/conditions?q=${encodeURIComponent(q)}`, { cache: "no-store" });
