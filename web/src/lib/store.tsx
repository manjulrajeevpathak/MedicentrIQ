...
import type { AccessState, AuditEvent, DashboardData, DirectoryPatient, InboxItem, PatientSummary, PermissionKey, QueuePriority } from "./types";
...
type AppContextValue = {
  ...
  /** Replaces inbox conversations with fresh server data (background polling). */
  refreshInbox: (items: InboxItem[]) => void;
...
export function AppProvider({ initial, children }: { initial: DashboardData; children: ReactNode }) {
  ...
  const refreshInbox = useCallback((items: InboxItem[]) => {
    setData((d) => ({ ...d, inbox: items }));
  }, []);
