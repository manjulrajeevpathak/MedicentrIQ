/* ---------------------------------------------------------------------------
   HealthFlow staff console — domain types
...
export type InteractionMessage = {
  ...
  body: string;
  internal?: boolean;
  /** Outbound authorship — distinguishes the AI assistant from human staff. */
  origin?: "assistant" | "staff" | "workflow";
  /** Delivery state for outbound messages — drives the WhatsApp-style ticks. */
  status?: "sent" | "failed" | "delivered" | "read" | "received";
  /** Present when the message carried an attachment. */
  media?: { kind: "image" | "document"; filename?: string };
};
...
export type InboxItem = {
  ...
  channel: Channel;
  patient: string;
  /** Patient's WhatsApp/phone number this conversation runs on. */
  phone?: string;
  /** ISO time of the patient's latest inbound message — anchors Meta's 24h reply window. */
  lastPatientAt?: string;
  preview: string;
  intent: string;
