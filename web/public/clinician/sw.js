...
const SHELL = ["/clinician", "/clinician/manifest.webmanifest", "/clinician/icon-192.png", "/clinician/icon-512.png"];
...
self.addEventListener("fetch", (event) => {
  ...
  event.respondWith(
    fetch(request)
      ...
      .catch(() => caches.match(request).then((cached) => cached || caches.match("/clinician")))
  );
});
